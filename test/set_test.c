...
int main()
{
    Set* int_set = set_create(sizeof(int), compare_ints, hash_int);
    for (int i = 0; i < 10; i++) {
        set_insert(int_set, &i);
    }

    Set* str_set = set_create(sizeof(char*), compare_strings, hash_string);
    const char* words[] = {"apple", "banana", "cherry", NULL};
    for (int i = 0; words[i]; i++) {
        set_insert(str_set, &words[i]);
    }

    Set* setA = set_create(sizeof(int), compare_ints, hash_int);
    Set* setB = set_create(sizeof(int), compare_ints, hash_int);
